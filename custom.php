...
function dc_secondary_nav($type='items'){
	if($type=='items'){
		$links=array(
			array('label'=>'All Items','class'=>'browse button','uri'=>'/items/browse'),
			array('label'=>'Item Tags','class'=>'tags button','uri'=>'/items/tags'),
			);
	}elseif($type=='collections'){
		$links=array(
			array('label'=>'All Collections','class'=>'browse button','uri'=>'/collections/browse')
			);
	}elseif($type=='exhibits'){
		$links=array(
			array('label'=>'All Exhibits','class'=>'active button','uri'=>'/exhibits'),
			array('label'=>'Exhibit Tags','class'=>'search button','uri'=>'/exhibits/tags')
			);
	}elseif($type=='search-results'){
		$links=array(
			array('label'=>'Site Search','class'=>'active button','uri'=>'/search'),
			array('label'=>'Item Search','class'=>'search button','uri'=>'/items/search')
			);	
	}elseif($type=='item-map'){
		$links=array(
			array('label'=>'Items Map','class'=>'map button','uri'=>'/items/map'),
			array('label'=>'All Items','class'=>'map button','uri'=>'/items/browse')
			);
	}else{
		$links=array(
			array('label'=>'Browse All','class'=>'browse button','uri'=>'#')
			);
	}		
	return nav($links);	
}
...
function do_quick_link($option=null){
	if($option){
		switch($option){
			case 'items':
			return '<a href="/items/browse/" class="button quick-link">'.__('Items').'</a>';
			case 'collections':
			return '<a href="/collections/browse/" class="button quick-link">'.__('Collections').'</a>';
			case 'exhibits':
			return '<a href="/exhibits/browse/" class="button quick-link">'.__('Exhibits').'</a>';
			case 'map':
			return '<a href="/items/map/" class="button quick-link">'.__('Map').'</a>';
			case 'about':
			return '<a href="/about/" class="button quick-link">'.__('About').'</a>';
			case 'contact':
			return '<a href="/contact/" class="button quick-link">'.__('Contact').'</a>';																				
			case 'contribute':
			return '<a href="/contribution/" class="button quick-link">'.__('Contribute').'</a>';	
			default:
			return null;
		}
	}
...
function the_hyperlink($record,$markup=true){
	$user_url = metadata($record,array('Item Type Metadata','URL'));
	$url = $user_url ? trim(strip_tags($user_url)) : null;
	$hostname = parse_url($url, PHP_URL_HOST);
	if($url && $hostname){
		$html='<a class="button button-primary" target="_blank" href="'.$url.'">'
		.'View @ '.$hostname.'</a><div>Or scroll down for additional info</div>';
	}else{
...
function isVideoJS($mime){
		$video = array('video/mp4','video/mpeg','video/ogg','video/quicktime','video/webm');	
...
function get_the_authors($item){
	...
	if ($creators = metadata($item,array('Dublin Core', 'Creator'),array('all'=>true))){
		$authArray=array();
		...
		switch($authTotal){
			...
			case 2:
			$authors='<span class="auth">'.$authArray[0].'</span> <span class="amp">&amp;</span> <span class="auth">'.$authArray[1].'</span>';
			break;		
	
			default:
			$authors='<span class="auth">'.$authArray[0].'</span>, <span class="auth">'.$authArray[1].'</span>, et al.';
		}
...
function homepage_grid_recent_items($num=10){ 
	$items=get_records('Item',array('sort_field' => 'added', 'sort_dir' => 'd'), $num);
	if($items){
...
function homepage_grid_featured_collections($num=1){ 
	$collections=get_records('Collection',array('sort_field' => 'added', 'sort_dir' => 'd', 'featured'=>true), $num);
	if($collections){
	$i=1;
...
function homepage_grid_featured_exhibit($num=1){ 
	if(plugin_is_active('ExhibitBuilder')){
		$exhibits=get_records('Exhibit',array('sort_field' => 'added', 'sort_dir' => 'd', 'featured'=>true), $num);
		if($exhibits){
		$i=1;
...
function flex_grid_item($item){
		...
		$itemclass ='';
			$itemclass .= ($typeName) ? ' '.strtolower(str_replace(' ', '-', $typeName)) : null;
		...
		<article class="grid item<?php echo $itemclass ;?>">
			<div class="inner flex-2">				
				...
				<a class="thumbnail" href="<?php echo record_url($item);?>"><?php echo ($img=item_image('square_thumbnail',array(),0,$item)) ? $img : null;?></a>
				...
				<div class="item-info">
					...
					if($collection=metadata('item','Collection Name')){
						...
						$headmeta[]='<span class="item-meta-tags">Filed under: '.tag_string('item','items/browse',', ').'</span>';
					}
...
function flex_grid_other_search_results($record,$recordType){
		...
		<article class="grid item<?php echo strtolower($recordType) ;?>">
			<div class="inner flex-2">				
				...
				<div class="item-info">
					<h3><a href="<?php echo record_url($record);?>"><?php echo metadata($record,array('Dublin Core','Title'));?></a></h3>
...
function flex_grid_collection($collection,$i){
		$item=get_record('Item', array('hasImage'=>true,'collection'=>$collection,'sort_field' => 'added', 'sort_dir' => 'd'));		
		$title=metadata($collection,array('Dublin Core','Title'));
		$description=metadata($collection,array('Dublin Core','Description'));
		$snippet=snippet($description,0, 400, '... ');
		$cta='View collection';
		$text = '<h3><a href="'.record_url($collection).'">'.$title.'</a></h3>'.
		'<p>'.$snippet.'</p>'.
		'<a href="'.record_url($collection).'" class="button">'.$cta.'</a>';		
		?>	
			<article class="collection">
				<div class="flex-2">
					<div class="col"><?php echo $text;?></div>
					<div class="col">
						<?php echo '<a class="collection-img" href="'.record_url($collection).'">'.item_image('fullsize',array(),0,$item).'</a>';?>
					</div>
				</div>
			</article>		
		<?php 	
...
function flex_grid_exhibit($exhibit,$i){
		...
		$description=($exhibitDescription = metadata($exhibit, 'description', array('no_escape' => true))) ? $exhibitDescription : null;
		$snippet=snippet($description,0, 400, '... ');
		$exhibitCoverImageFile = ($id=$exhibit->cover_image_file_id) ? get_record_by_id('File',$id) : null;
		$text = '<h3>'.$title.'</h3>'.
		'<p>'.$snippet.'</p>'.
		'<a href="'.record_url($exhibit).'" class="button">View Exhibit</a>';		
		?>	
			<article class="collection">
				<div class="flex-2">
					<div class="col"><?php echo $text;?></div>
					<div class="col">
						<?php echo $exhibitCoverImageFile ? '<a class="collection-img" href="'.record_url($exhibit).'">'.record_image($exhibitCoverImageFile,'fullsize').'</a>' : null;?>
					</div>
				</div>
			</article>		
		<?php
...
function footer_text_snippets($length=250){ ?>
			<div class="col"><h2><span>About</span> This</h2>
				<article>
				<?php if($about=get_theme_option('footer_about_text')){
					$about= snippet($about, 0, $length, '...');
					echo '<p>'.$about.'</p><a class="read-more button" href="'.WEB_ROOT.'/about">Read More</a>';
				}else{
			...
			<div class="col"><h2><span>About</span> That</h2>
				<article>
				<?php if($sponsorAbout=get_theme_option('footer_sponsor_text')){
					$sponsorAbout= snippet($sponsorAbout, 0, $length, '...');
					echo '<p>'.$sponsorAbout.'</p><a class="read-more button" href="#">Visit Our Homepage</a>';
				}else{
					echo '<p>You have not entered any text in theme settings.</p>';
				}?>
				</article>
...
function gallery_script($num=6){
	...
	$items=get_records('Item',array('sort_field' => 'modified', 'sort_dir' => 'd', 'featured'=>true,'hasImage'=>true), $num);
	if($items){
		$itemsArray=[];
		foreach($items as $item){ 
			if($img=get_hero_for_item($item,'fullsize')){
				$itemsArray[]=array(
					...
					'fullsize'=>$img,
					'title'=>($title=metadata($item,array('Dublin Core','Title'))) ? $title : 'No Title',	
					'creator'=>($creator=metadata($item,array('Dublin Core','Creator'))) ? $creator : 'Creator Unknown',
					'date'=>($date=metadata($item,array('Dublin Core','Date'))) ? $date : 'Date Unknown',		
					'collection'=>( $collection=get_collection_for_item($item) ) ? link_to($collection,'show','View Collection',array('class'=>'cta')) : '',
					);
				}
			}
		if(count($itemsArray) > 0){
			...
			<div class="banner">
				<ul>
					...
					<li>
						<div class="slide-container" style="background:url(<?php echo $i['fullsize'];?>) no-repeat;background-size: cover;background-position: center">
							<div class="slide-info">
								<h1><?php echo '<a href="'.$i['url'].'">'.$i['title'].'</a>';?></h1>
								<h2><?php echo $i['date'];?> / <?php echo $i['creator'];?></h2>
...
function browse_by_item_type(){
	$types = get_records('ItemType',array(),0);
	$opts=array();
	foreach($types as $type){
		$total=$type->totalItems();
		if($total>0){
			$opts[$type->name] = '<option value="'.$type->id.'">'.$type->name.' ('.$total.')</option>';
			}
	}
	ksort($opts);
...
function random_item_link($text=null,$class='show'){
	...
	if( count( $randitems ) > 0 ){
		...
		$link = link_to( 'items', 'browse', 'Browse Items',
			array( 'class' => $linkclass ) );
	}
	return $link;
}
	...
	foreach (loop('files', $item->Files) as $file){ 
		...
		if ( isImg($mime) !== false ){
		    ...
		    $html.='<figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject">';
			    $html.='<a href="'.($custom_url ? $custom_url : file_display_url($file,'fullsize')).'" itemprop="contentUrl" data-size="" '.($custom_url ? 'target="_blank"' : null).'>';
			    	$html.='<img src="'.file_display_url($file,'fullsize').'" itemprop="thumbnail" alt="'.metadata($file,array('Dublin Core', 'Title')).'" />';
			    $html.='</a>';
				$html.='<figcaption itemprop="caption description">'.create_file_caption($file).'</figcaption>';
		    $html.='</figure>';	
	
		}
	}
	if($html){
		return '<div class="pswp-image-gallery" itemscope itemtype="http://schema.org/ImageGallery">'.$html.'</div>';
	}else{
		return null;
	}
	...
	foreach (loop('files', $item->Files) as $file):
		...
		if (isVideoJS($mime) !== false) { ?>
			...
			<div class="video-container" id="v<?php echo $index;?>">
				<video id="video_<?php echo $index;?>" class="video-js vjs-default-skin video" controls playsinline preload="auto" data-setup="{}">
				 	<source src="<?php echo file_display_url($file,'original');?>" type='video/mp4'/>
				 	Your browser doesn't support HTML &lt;video&rt;. <a href="<?php echo file_display_url($file,'original');?>">Download the file</a>.
				</video>
			</div>
		...
		<?php }elseif (isMP3($mime) !== false){ ?>	
			<div class="audio-container" id="a<?php echo $index;?>">
				...
				controls >
					<source 
					src='<?php echo file_display_url($file,'original');?>' 
					type="<?php echo $mime ;?>"/>
						        
			        Your browser doesn't support HTML &lt;audio&rt;. 
			        <a href="<?php echo file_display_url($file,'original');?>">Download the file</a>.
				</audio>
			</div>		
		<?php }
		$index++;		
...
function create_file_caption($file){
	$captionTitle = (metadata($file,array('Dublin Core', 'Title'))) ? '<strong>'.metadata($file, array('Dublin Core', 'Title')).'</strong>' : '';		
	$captionCreator = (metadata($file,array('Dublin Core', 'Creator'))) ? metadata($file, array('Dublin Core', 'Creator')) : '';
	$captionDate = (metadata($file,array('Dublin Core', 'Date'))) ? metadata($file, array('Dublin Core', 'Date')) : '';
	if($captionCreator || $captionDate){
		$captionTitle.='<br>'.implode(' | ',array($captionCreator,$captionDate));	
	}
	$captionDescription = (metadata($file,array('Dublin Core', 'Description'))) ? ': '.metadata($file, array('Dublin Core', 'Description')) : '';
	$captionSource = (metadata($file,array('Dublin Core', 'Source'))) ? '<div>Source: '.metadata($file, array('Dublin Core', 'Source')).'</div>' : '';
	$captionLink = (metadata($file,'id')) ? '<div class="more-info">More Info: '.link_to($file,'show', 'View file metadata',array('class'=>'view-file-record','rel'=>'nofollow')).'</div>' : '';
	
	$caption=$captionTitle.$captionDescription.$captionSource.$captionLink;
...
function file_download_table($item=null, $html=null){
	if($files=loop('files', $item->Files)){
		...
		$html.='<thead><tr><th>'.__('Name').'</th><th>'.__('Info').'</th><th>'.__('Download').'</th></tr></thead><tbody>';
		foreach ($files as $file){ 		
			$file_url=file_display_url($file,'original');
			$file_title=((metadata($file,array('Dublin Core', 'Title'))) ? metadata($file, array('Dublin Core', 'Title')) : $file->original_filename);
			$file_link='<a class="button download-file-button" href="'.$file_url.'" download="'.$file->original_filename.'">Download</a>';
			$file_info='<div class="download-file-info"><span>'.$file->getExtension().'</span> / '.formatSizeUnits($file->size).'</div>';
			$html.='<tr><td>'.$file_title.'</td><td>'.$file_info.'</td><td>'.$file_link.'</td></tr>';
		}
		$html.='</tbody></table>';
		return $html;
	}
...
function social_links_formatted($html=null){
	...
	if(($email) || ($facebook) || ($twitter) || ($instagram) || ($youtube) || ($pinterest) ){
		$html.='<div class="social-media-links">';
            ...
            $html.= $email ? '<a target="_blank" title="Email" class="fa fa-envelope hcard" href="'.$email.'"></a>' : null;
            $html.= $facebook ? '<a target="_blank" title="Facebook" class="fab fa-facebook hcard" href="'.$facebook.'"></a>' : null;
            $html.= $twitter ? '<a target="_blank" title="Twitter" class="fab fa-twitter hcard" href="'.$twitter.'"></a>' : null;
            $html.= $instagram ? '<a target="_blank" title="Instagram" class="fab fa-instagram hcard" href="'.$instagram.'"></a>' : null;
            $html.= $pinterest ? '<a target="_blank" title="Pinterest" class="fab fa-pinterest hcard" href="'.$pinterest.'"></a>' : null;
            $html.= $youtube ? '<a target="_blank" title="Youtube" class="fab fa-youtube hcard" href="'.$youtube.'"></a>' : null;
