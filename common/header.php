...
<header id="main">       
	<div class="header-container">           
		...
		<nav id="primary-nav">
			<?php echo do_quick_link(get_theme_option('quicklinks_1'));?>
			<?php echo do_quick_link(get_theme_option('quicklinks_2'));?>
...
<?php if(is_current_url('/')): ?>
	<div id="banner" style="<?php echo ($img=get_theme_option('homepage_cta_img')) ? 'background-image: url('.WEB_ROOT.'/files/theme_uploads/'.$img.');' : '' ;?>">
		...
		($cta_header=get_theme_option('homepage_cta_header'))
			...
			echo '<a class="button" href="'.$cta_url.'">'.strip_tags($cta_button).'</a>';
			echo '</article>';
		}
		?>
		<br></div>
