<?php echo $this->form('search-form', $options['form_attributes']); ?>
    <?php echo $this->formText('query', $filters['query'], array('title' => __('Search'), 'placeholder'=>__('Enter a search term'))); ?>
    <?php if ($options['show_advanced']): ?>
    <div id="advanced-form" class="flex-3">
	    <div class="col">
	        <fieldset id="query-types">
	            <legend><?php echo __('Search using this query type:'); ?></legend>
	            <?php $t= $this->formRadio('query_type', $filters['query_type'], null, $query_types); ?>
	            <?php echo str_replace(array('<br>','<br/>'),' ', $t);?>
	        </fieldset>
	    </div>
	    <div class="col">
	        <?php if ($record_types): ?>
    ...
    <div>
	    ...
	    <?php if ($options['show_advanced']): ?>
	    	<div>Or go to <?php echo link_to_item_search(__('Advanced Search')); ?> (items only)</div>
	    <?php endif; ?>
    </div>
</form>
